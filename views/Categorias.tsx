...
import { 
  Tags, Search, Plus, Edit3, Trash2, X, Save, 
...
export const CategoriasView: React.FC = () => {
  ...
  const handleOpenEdit = (cat: Category) => {
    ...
    setFormData({ codigo: cat.codigo, nome: cat.nome });
    setIsModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingCategory) {
      ...
      addCategory({ ...formData, id: '', tenant_id: '' });
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Categorias</h2>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Classificação de associados e serviços</p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[32px] overflow-hidden shadow-sm">
        <div className="px-8 py-5 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/20">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
            <ListIcon size={14} /> Lista de Registros
          </h3>
          <span className="text-[9px] font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-full uppercase">{(filteredCategories || []).length} categorias</span>
        </div>
        <table className="w-full text-left">
          <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
            <tr>
              <th className="px-8 py-5 text-[9px] font-black uppercase text-slate-400 tracking-widest">Código</th>
              <th className="px-8 py-5 text-[9px] font-black uppercase text-slate-400 tracking-widest">Nome da Categoria</th>
              <th className="px-8 py-5 text-[9px] font-black uppercase text-slate-400 tracking-widest text-right">Ações</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {(filteredCategories || []).map(cat => (
              <tr key={cat.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors group">
                <td className="px-8 py-5 w-48">
                  <span className="font-mono text-xs text-blue-600 font-bold bg-blue-50 dark:bg-blue-900/40 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800/50">{cat.codigo}</span>
                </td>
                <td className="px-8 py-5">
                   <p className="font-black text-xs uppercase text-slate-800 dark:text-slate-200">{cat.nome}</p>
                </td>
                <td className="px-8 py-5 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => handleOpenEdit(cat)} className="p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-white dark:hover:bg-slate-700 transition-all border border-transparent hover:border-blue-100 dark:hover:border-blue-900"><Edit3 size={16}/></button>
                    <button onClick={() => { setCategoryToDelete(cat); setIsDeleteModalOpen(true); }} className="p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-400 hover:text-red-500 hover:bg-white dark:hover:bg-slate-700 transition-all border border-transparent hover:border-red-100 dark:hover:border-red-900"><Trash2 size={16}/></button>
                  </div>
                </td>
              </tr>
            ))}
            {(filteredCategories || []).length === 0 && (
              <tr>
                <td colSpan={3} className="px-8 py-20 text-center">
                   <Tags size={40} className="mx-auto text-slate-200 dark:text-slate-800 mb-4" />
                   <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Nenhuma categoria encontrada</p>
                </td>
              </tr>
            )}
          </tbody>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[48px] shadow-2xl relative z-10 overflow-hidden border border-white/10">
            <header className="p-10 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Defina as informações básicas de classificação</p>
              </div>
            ...
            <div className="p-10 space-y-6">
              ...
              <Input 
                ...
                placeholder="Ex: Pescador Artesanal" 
              />
            </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-md rounded-[40px] shadow-2xl relative z-10 overflow-hidden border border-red-100 dark:border-red-900/30">
            <div className="p-10 text-center">
              ...
              <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-4">Excluir Categoria?</h3>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-relaxed">
                Você está prestes a remover <span className="text-red-600 font-black">"{categoryToDelete?.nome}"</span>. Esta ação removerá o registro localmente e no banco de dados sincronizado.
              </p>
            </div>
